...
RSpec.describe HatiJsonapiError::Errors::HelpersHandleError do
  describe '.new' do
    context 'with default message' do
      subject(:error) { described_class.new }

      it 'creates an error with default message' do
        expect(error.message).to eq('Invalid Helpers:handle_error')
      end
    ...
    context 'with custom message' do
      ...
      let(:custom_message) { 'Custom handle error message' }
  ...
  describe 'error handling' do
    ...
    it 'can be raised with custom message' do
      custom_message = 'Handle error test message'
  ...
  describe 'error properties' do
    ...
    it 'maintains error class information' do
      aggregate_failures 'class' do
        ...
        expect(error.class.name).to eq('HatiJsonapiError::Errors::HelpersHandleError')
      end
    end
  end
  ...
  describe 'module structure' do
    ...
    it 'is accessible through module path' do
      expect(HatiJsonapiError::Errors::HelpersHandleError).to eq(described_class)
    end
  end

  describe 'usage in helpers context' do
    it 'provides meaningful error for handle_error method failures' do
    ...
    it 'can differentiate from render errors' do
      handle_error = described_class.new
      render_error = HatiJsonapiError::Errors::HelpersRenderError.new

      aggregate_failures 'class' do
        expect(handle_error.class).not_to eq(render_error.class)
        expect(handle_error.message).not_to eq(render_error.message)
      end
    end
  end
end
